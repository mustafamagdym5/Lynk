...
class ActiveDeliveries(LoginRequiredMixin, View):
    def get(self, request):
        ...
        for delivery in active_deliveries:
            # Debug print
            print(f"Delivery {delivery.id} - Customer: {delivery.sender.username}")
            print(f"Customer location: {delivery.sender.latitude}, {delivery.sender.longitude}")
            ...
            locations.append({
                'type': 'customer',
                'lat': delivery.sender.latitude,
                'lng': delivery.sender.longitude,
                'name': f"Customer: {delivery.sender.username}"
            })
            
            cart_products = CartProducts.objects.filter(cart=delivery.cart).select_related('product__owner')
            for cp in cart_products:
                vendor = cp.product.owner
                print(f"Vendor {vendor.username} location: {vendor.latitude}, {vendor.longitude}")
                locations.append({
                    'type': 'vendor',
                    'lat': vendor.latitude,
                    'lng': vendor.longitude,
                    'name': f"Vendor: {vendor.username}"
                })
