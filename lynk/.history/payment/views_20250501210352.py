...
class CheckOutView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        product_cost = sum(cart_product.product.price * cart_product.quantity 
    ...
    def get(self, request):
        ...
        if checkout_session_id:
            try:
                ...
                delivery_request_id = session.metadata.get('delivery_request_id')
                if delivery_request_id:
                    delivery_request = Request.objects.get(id=delivery_request_id)
                    delivery_request.payment_status = 'completed'
                    delivery_request.stripe_payment_intent = session.payment_intent
