...
class ProductView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        for item in cart_products:
            # Assuming each item.product has a related Stripe price ID stored
            line_items.append({
                'price': item.product.stripe_price_id,  # you need to store this in your Product model
                'quantity': item.quantity,
            })

        if not line_items:
            return redirect('cart:cart_detail')  # Or any fallback
