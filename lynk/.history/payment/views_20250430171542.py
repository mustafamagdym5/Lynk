...
class ProductView(LoginRequiredMixin, View):
    def get(self, request):
        try:
            ...
            for item in cart_products:
                # Get or create StripeProduct entry
                ...
                if created or not stripe_product.stripe_product_id:
                    ...
                    product = stripe.Product.create(
                        ...
                        images=[item.product.product_image.url] if item.product.product_image else []
                    )

                    # Create price in Stripe
                    price = stripe.Price.create(
                        product=product.id,
                        unit_amount=int(item.product.get_discounted_price() * 100),  # Assuming this method exists
                        currency='egp',
                    )
            ...
            context = {
                ...
                'total': sum(item.product.get_discounted_price() * item.quantity for item in cart_products),
            }
    ...
    def post(self, request):
        ...
        try:
            ...
            line_items = []
            for item in cart_products:
                stripe_product = StripeProduct.objects.get(item=item.product)
                line_items.append({
                    'price': stripe_product.stripe_price_id,
                    'quantity': item.quantity,
                })
            ...
            if checkout_session_id:
                ...
                for line_item in line_items:
                    UserPayment.objects.get_or_create(
                        ...
                        product_name=line_item.description or "Unknown Product",
