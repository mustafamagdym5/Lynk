...
class SelectDelivery(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if not delivery_id:
            ...
            return redirect('customer:select_delivery')
        ...
        try:
            ...
            return redirect('customer:waiting_for_approval', request_id=delivery_request.id)
...
class SendRequestToDelivery(LoginRequiredMixin, View):
    def post(self, request):
        delivery_id = request.POST.get('delivery_person')
        if not delivery_id:
            return redirect('customer:select_delivery')
        ...
        # Get the distance and cost from the form submission
        distance = Decimal(request.POST.get('distance', 0))
        cost = Decimal(request.POST.get('cost', 0))
